...
def grid_sample(
    pos, batch, size, start, return_p2v=True, return_counts=True, return_unique=False
):
    # pos: float [N, 3]
    # batch: long [N]
    # size: float [3, ]
    ...
    cluster = voxel_grid(pos, size, batch, start=start)  # [N, ]
...
def get_indices_params(xyz, batch, window_size, shift_win: bool):
    ...
    if shift_win:
        ...
        v2p_map, k, counts = grid_sample(
            xyz, batch, window_size, start=None, return_p2v=False, return_counts=True
        )
    ...
    n_max = k
    index_0_offsets, index_1_offsets, index_0, index_1 = precompute_all(
        N, n, n_max, counts
    )
...
def scatter_softmax_csr(src: torch.Tensor, indptr: torch.Tensor, dim: int = -1):
    ...
    sum_per_index = segment_csr(recentered_scores_exp, indptr, reduce="sum")
