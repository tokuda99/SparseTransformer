...
def _trunc_normal_(tensor: torch.Tensor, mean: float, std: float, a: float, b: float):
    ...
    l_cdf = norm_cdf((a - mean) / std)
    u_cdf = norm_cdf((b - mean) / std)

    tensor.uniform_(2 * l_cdf - 1, 2 * u_cdf - 1)
...
def sparse_self_attention(
    ...
    if pe_type == "contextual" and rel_query and rel_key:
        ...
        xyz_quant = torch.div(
            xyz_quant,
            torch.from_numpy(quant_size).float().cuda(),
            rounding_mode="floor",
        )  # [N, 3]
        relative_position = (
            xyz_quant[index_0.long()] - xyz_quant[index_1.long()]
        )  # [M, 3]
...
class VarLengthMultiheadSA(nn.Module):
    ...
    def forward(self, sptr_tensor: SparseTrTensor):
        ...
        if self.pe_type in ["sine", "fourier"]:
            ...
            assert (
                offset.shape[0] == batch.max().item() + 1
            ), "batch must be sorted and consecutive"
        ...
        if index_params is None:
            ...
            assert (
                (window_size == self.window_size) and (shift_win == self.shift_win)
