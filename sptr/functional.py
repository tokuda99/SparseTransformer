...
class AttentionStep1(Function):
    ...
    def forward(ctx, q, k, index0, index0_offsets, index1, index1_offsets, n_max):
        ...
        sptr_cuda.attention_step1_forward_cuda(
            N_q,
            N_k,
            M,
            h,
    ...
    def backward(ctx, grad_output):
        ...
        assert (
            ...
            and index0_offsets.is_contiguous()
            and index1.is_contiguous()
            and index1_offsets.is_contiguous()
        ...
        sptr_cuda.attention_step1_backward_cuda(
            N_q,
            M,
            h,
            hdim,
...
class AttentionStep2(Function):
    ...
    def forward(ctx, attn, v, index0, index0_offsets, index1, index1_offsets, n_max):
        ...
        sptr_cuda.attention_step2_forward_cuda(
            N, M, h, hdim, n_max, attn, v, index0_offsets, index1, output
        )

        if DEBUG:
            torch.cuda.synchronize()
            print(
                "attn_step2: forward time: {}s, M: {}, h: {}".format(
                    time.time() - t, M, h
                )
            )
    ...
    def backward(ctx, grad_output):
        ...
        sptr_cuda.attention_step2_backward_cuda(
            N,
            M,
            h,
            hdim,
        ...
        if DEBUG:
            torch.cuda.synchronize()
            print(
                "attn_step2: backward time: {}s, M: {}, h: {}".format(
                    time.time() - t, M, h
                )
            )
...
class DotProdWithIdx(Function):
    ...
    def forward(
        ...
        assert L > rel_idx.max(), "L = {}, while rel_idx.max() = {}".format(
            L, rel_idx.max()
        )
        assert L <= 50
        ...
        sptr_cuda.dot_prod_with_idx_forward_cuda(
            N,
            M,
            h,
            hdim,
        ...
        if DEBUG:
            torch.cuda.synchronize()
            print(
                "dot_prod_with_idx: forward time: {}s, M: {}".format(time.time() - t, M)
            )

        ctx.n_max = n_max
        ctx.save_for_backward(
            q, index_q_offsets, k, index_k_offsets, index_k, table_q, table_k, rel_idx
        )
        return output
    ...
    def backward(ctx, grad_output):
        ...
        assert (
            ...
            and index_q_offsets.is_contiguous()
            and k.is_contiguous()
            and index_k_offsets.is_contiguous()
            and index_k.is_contiguous()
            and table_q.is_contiguous()
            and table_k.is_contiguous()
            and rel_idx.is_contiguous()
        ...
        sptr_cuda.dot_prod_with_idx_backward_cuda(
            N,
            M,
            h,
            hdim,
        ...
        if DEBUG:
            torch.cuda.synchronize()
            print(
                "dot_prod_with_idx: backward time: {}s, M: {}".format(
                    time.time() - t, M
                )
            )
...
class DotProdWithIdxAll(Function):
    ...
    def forward(
        ...
        sptr_cuda.dot_prod_with_idx_all_forward_cuda(
            N,
            M,
            h,
            hdim,
        ...
        if DEBUG:
            torch.cuda.synchronize()
            print(
                "dot_prod_with_idx_all: forward time: {}s, M: {}".format(
                    time.time() - t, M
                )
            )
    ...
    def backward(ctx, grad_output):
        ...
        if DEBUG:
            torch.cuda.synchronize()
            print(
                "dot_prod_with_idx_all: backward time: {}s, M: {}".format(
                    time.time() - t, M
                )
            )
...
class AttentionStep2WithRelPosValue(Function):
    ...
    def forward(
        ...
        sptr_cuda.attention_step2_with_rel_pos_value_forward_cuda(
            N,
            M,
            h,
            hdim,
        ...
        if DEBUG:
            torch.cuda.synchronize()
            print(
                "attn_step2_with_rel_pos_value: forward time: {}s, M: {}, h: {}".format(
                    time.time() - t, M, h
                )
            )
        ...
        ctx.n_max = n_max
        ctx.save_for_backward(
            attn, v, index0, index0_offsets, index1, index1_offsets, table, rel_idx
        )
        return output
    ...
    def backward(ctx, grad_output):
        ...
        sptr_cuda.attention_step2_with_rel_pos_value_backward_cuda(
            N,
            M,
            h,
            hdim,
        ...
        if DEBUG:
            torch.cuda.synchronize()
            print(
                "attn_step2_with_rel_pos_value: backward time: {}s, M: {}, h: {}".format(
                    time.time() - t, M, h
                )
            )
